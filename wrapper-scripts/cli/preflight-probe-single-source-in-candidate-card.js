...
const usageLines = [
  'Usage:',
  '  cat payload.json | node preflight-probe-single-source-in-candidate-card.js --source-idx <n>',
  '',
...
function main() {
  ...
  log_it('preflight-probe-single-source-in-candidate-card.js', argv.join(' '));
  ...
  const result = runBoardLiveCardsCli('probe-source-preflight', ['--source-idx', sourceIdx], payload);
  printJson(result);
}
