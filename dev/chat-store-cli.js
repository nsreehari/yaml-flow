...
const distCli = path.join(repoRoot, 'dist', 'cli', 'node', 'chat-store-cli.js');
...
if (fs.existsSync(srcCli)) {
  ...
  if (result.error) {
    console.error(`[chat-store] Failed to launch dev fallback: ${result.error.message}`);
    process.exit(1);
  }
  ...
  console.error('[chat-store] Could not find dist or src CLI entrypoint.');
  process.exit(1);
}
