...
const distCli = path.join(repoRoot, 'dist', 'cli', 'node', 'batch-runner-cli.js');
const srcCli = path.join(repoRoot, 'src', 'cli', 'node', 'batch-runner-cli.ts');
...
if (fs.existsSync(srcCli)) {
  ...
  console.error('[batch-runner-cli] Could not find dist or src CLI entrypoint.');
  process.exit(1);
}
