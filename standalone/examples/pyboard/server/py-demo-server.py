...
_PYCLI_ROOT = os.path.normpath(os.path.join(__file_dir, "..", "..", "..", "core"))
...
def create_fs_board_platform_adapter(base_ref: Dict[str, str], notify_channel: Optional[str] = None):
    ...
    class _Adapter:
        ...
        def self_ref(self) -> Dict[str, Any]:
            board_pycli = os.path.join(_PYCLI_ROOT, "cli", "board_live_cards_pycli.py")
            ref: Dict[str, Any] = {
...
def create_file_notification_transport(poll_interval_s: float = 0.1):
    ...
    Subprocess callbacks append JSONL records to a shared file path (notify channel).
    This transport tails that file and forwards events to the runtime subscription hook.
    """
            ...
            def _tail() -> None:
                ...
                while not stop.is_set():
                    try:
                        ...
                        # Keep tailing despite transient read/parse errors.
                        pass
...
def build_board_context_config(label, board_dir, task_exec_path, chat_handler_path_, inf_adapter_path, board_id_):
    ...
    # Truncate on startup so subscriptions only consume fresh events.
