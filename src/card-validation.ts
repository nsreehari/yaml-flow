...
export interface CardPreflightResult {
  isValid: boolean;
  issues: string[];
}
 ...
 */
export function validateCardPreflight(card: unknown): CardPreflightResult {
  const result = validateLiveCardDefinition(card);
  return { isValid: result.ok, issues: result.errors };
}
