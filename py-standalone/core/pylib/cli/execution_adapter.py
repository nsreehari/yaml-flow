...
def invoke_ref_sync(
    ...
    Behavior mirrors src/cli/node/execution-adapter.ts:invokeRefSync:
      1. Resolve ref.argsMassaging against args -> cmdArgs / stdin.
      2. Build the local base spec (node/python/process + script path).
      3. Spawn synchronously with json(stdin or args) on stdin.
    ...
    stdin_payload = json.dumps(
        massaged["stdin"] if "stdin" in massaged else args,
        ensure_ascii=True,
    )
