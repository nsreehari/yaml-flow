...
_CLI_PARSER.add_argument('--port', type=int, default=None, help='Server port (unused in CLI mode)')
_CLI_ARGS = _CLI_PARSER.parse_args()

# ── Path resolution ────────────────────────────────────────────────────────────
_HERE = os.path.dirname(os.path.abspath(__file__))
_YAML_FLOW_ROOT = os.path.normpath(os.path.join(_HERE, '..', '..', '..'))
...
BOARD_PYCLI = os.path.join(_YAML_FLOW_ROOT, 'py-standalone', 'core', 'cli', 'board_live_cards_pycli.py')
CARD_STORE_PYCLI = os.path.join(_YAML_FLOW_ROOT, 'py-standalone', 'core', 'cli', 'card_store_pycli.py')
