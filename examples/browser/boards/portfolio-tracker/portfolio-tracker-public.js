...
async function waitForCompleted(label, timeoutMs = 90_000, pollMs = 500) {
  const required = new Set(['portfolio-form', 'price-fetch', 'holdings-table', 'portfolio-value']);
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    // Sleep first so any async drain() started by the preceding upsertCard/retrigger
    // gets to run and update status.json before we read it.
    await new Promise(r => setTimeout(r, pollMs));
    const result = makeBoard().status({});
    if (result.status === 'success') {
      const completed = new Set(
        result.data.cards.filter(c => c.status === 'completed').map(c => c.name)
      );
      if ([...required].every(n => completed.has(n))) {
...
for (const card of [
  setHoldings(CARD_PORTFOLIO_FORM, { AAPL: 50, MSFT: 30 }),
  CARD_PRICE_FETCH,
  CARD_HOLDINGS_TABLE,
...
const pricesT1 = readJson(pricesPath);
assert(typeof pricesT1 === 'object' && pricesT1 !== null && Object.keys(pricesT1).length > 0,
  'T1: prices.json is empty or not an object');
assert(JSON.stringify(Object.keys(pricesT1).sort()) === JSON.stringify(['AAPL', 'MSFT']),
  `T1: expected keys {AAPL, MSFT}, got ${JSON.stringify(Object.keys(pricesT1))}`);
assert(Object.values(pricesT1).every(v => typeof v === 'number'),
  'T1: all price values must be numbers');
console.log('[T1] assertion passed: prices.json has AAPL, MSFT with numeric values.');

// ── T2a — Update holdings (GOOG added) ────────────────────────────────────────
console.log('\n=== T2a: Update holdings (GOOG added) ===');
checkResult(
  makeCardStore().set({ body: setHoldings(CARD_PORTFOLIO_FORM, { AAPL: 50, MSFT: 30, GOOG: 100 }) }),
...
assert(JSON.stringify(Object.keys(pricesT2c).sort()) === JSON.stringify(['AAPL', 'GOOG', 'MSFT']),
  `T2c: expected keys {AAPL, MSFT, GOOG}, got ${JSON.stringify(Object.keys(pricesT2c))}`);

const htCvPath = path.join(OUTPUTS_DIR, 'cards', 'holdings-table', 'computed_values.json');
const htCvT2c = readJson(htCvPath);
assert(htCvT2c.table.rows.length === 3,
  `T2c: expected 3 rows in holdings-table, got ${htCvT2c.table.rows.length}`);
console.log('[T2c] assertions passed: 3 tickers in prices, 3 rows in holdings-table.');
...
assert(JSON.stringify(Object.keys(pricesT3).sort()) === JSON.stringify(['AAPL', 'GOOG', 'MSFT']),
  `T3: expected 3 tickers, got ${JSON.stringify(Object.keys(pricesT3))}`);
assert(JSON.stringify(pricesT3) !== JSON.stringify(pricesT2c),
  'T3: prices must differ from T2c values after retrigger');
console.log('[T3] assertions passed: 3 tickers, prices differ from T2c.');

// ── T4 — Rapid 5× portfolio-form updates ──────────────────────────────────────
console.log('\n=== T4: Rapid 5x portfolio-form updates ===');
for (const holdings of [
  { AAPL: 50, MSFT: 30, GOOG: 100, AMZN: 40 },
  { AAPL: 45, MSFT: 30, GOOG: 110, AMZN: 40, TSLA: 60 },
